...
_FP32_GUIDANCE_ENV = "LTX2_GUIDANCE_FP32"


def fp32_guidance_enabled() -> bool:
    """Whether the multi-modal guidance combination should run in fp32.

    The CFG/STG/modality combination is a sum of differences of nearly-equal
    predictions (``cond - uncond``) scaled by the guidance coefficients, plus a
    std-ratio rescale. In fp16 this loses precision via cancellation and the
    large-tensor std reduction, injecting high-frequency noise into the
    denoised latent. The transformer forward stays fp16; only this cheap
    elementwise combine is upcast.
    """
    return os.getenv(_FP32_GUIDANCE_ENV, "").lower() in {"1", "true", "yes", "on"}
...
class MultiModalGuider:
    ...
    def calculate(
        ...
        When ``LTX2_GUIDANCE_FP32`` is set, the combination (and the std-ratio
        rescale) are computed in fp32 and cast back to ``cond``'s dtype. This
        avoids fp16 cancellation in ``cond - uncond`` and precision loss in the
        large-tensor std reduction, which otherwise inject high-frequency noise
        at high guidance scales. The upstream transformer outputs are unchanged.
        """
