...
class AscendChunkedAttention(AttentionCallable):
    ...
    def __init__(self, query_chunk_size: int | None = None, eager_max_mb: int | None = None) -> None:
        self.query_chunk_size = query_chunk_size or int(os.getenv("LTX2_ASCEND_ATTENTION_CHUNK", "512"))
        self.eager_max_bytes = (eager_max_mb or int(os.getenv("LTX2_ASCEND_ATTENTION_EAGER_MAX_MB", "1024"))) * 1024 * 1024

    def _softmax(self, scores: torch.Tensor, out_dtype: torch.dtype) -> torch.Tensor:
    ...
    def __call__(
        ...
        if b * heads * q_len * k_len * torch.finfo(torch.float32).bits // 8 <= self.eager_max_bytes:
            ...
            if mask is not None:
                scores = scores + mask
            probs = self._softmax(scores, v.dtype)
            out = torch.matmul(probs, v)
            return out.transpose(1, 2).reshape(b, q_len, heads * dim_head)
...
def _select_primary_attention() -> AttentionCallable:
    ...
    """
    ascend_attn = os.getenv("LTX2_ASCEND_ATTENTION", "").lower()
    if ascend_attn == "fused":
        return AscendFusedAttention()
    if ascend_attn in {"eager", "math"}:
