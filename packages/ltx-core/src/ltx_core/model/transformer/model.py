...
def _contiguous_value(value):
    if isinstance(value, torch.Tensor):
        return value.contiguous()
    if isinstance(value, tuple):
        return tuple(_contiguous_value(item) for item in value)
    if isinstance(value, list):
        return [_contiguous_value(item) for item in value]
    if isinstance(value, dict):
        return {key: _contiguous_value(item) for key, item in value.items()}
    return value


def _modality_to_device_contiguous(modality, device: torch.device):
    moved = modality.to(device)
    if hasattr(moved, "_asdict"):
        return type(moved)(**{key: _contiguous_value(value) for key, value in moved._asdict().items()})
    return _contiguous_value(moved)
...
class LTXModel(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        block_device_map: dict[int, torch.device] | None = None,
    ):
        super().__init__()
        self._enable_gradient_checkpointing = False
        self.cross_attention_adaln = cross_attention_adaln
        self.block_device_map = block_device_map or {}
    ...
    def _init_transformer_blocks(
        ...
        for idx in range(num_layers):
            ...
            if idx in self.block_device_map:
                block = block.to(self.block_device_map[idx])
            blocks.append(block)
    ...
    def _process_transformer_blocks(
        ...
        dump_blocks = should_dump_tensor("transformer.block")

        for block_idx, block in enumerate(self.transformer_blocks):
            block_device = self.block_device_map.get(block_idx)
            if block_device is not None:
                if video is not None:
                    video = _modality_to_device_contiguous(video, block_device)
                if audio is not None:
                    audio = _modality_to_device_contiguous(audio, block_device)
            if dump_blocks:
                if video is not None:
                    dump_tensor(f"transformer.block.{block_idx}.video_in", video.x)
                if audio is not None:
                    dump_tensor(f"transformer.block.{block_idx}.audio_in", audio.x)
            if video is not None:
            ...
            setattr(block, "_debug_block_idx", block_idx)
            try:
                ...
                if hasattr(block, "_debug_block_idx"):
                    delattr(block, "_debug_block_idx")

            if dump_blocks:
                if video is not None:
                    dump_tensor(f"transformer.block.{block_idx}.video_out", video.x)
                if audio is not None:
                    dump_tensor(f"transformer.block.{block_idx}.audio_out", audio.x)
    ...
    def _process_output(
        ...
        output_device = proj_out.weight.device
