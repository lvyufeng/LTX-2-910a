...
bf16_fuse_rule = FuseRule(aggregation_dtype=torch.float16, fuse_fn=_bf16_fuse)
...
def _to_device_safe(tensor: torch.Tensor, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    """Move *tensor* to *device* as *dtype*, casting on the source device first.

    Ascend 910A has no bf16 kernels (not even allocation), so a bf16 tensor must
    never land on the NPU even transiently. ``Tensor.to(device, dtype)`` does not
    guarantee the cast happens before the H2D copy, so we cast on the current
    device first, then transfer.
    """
...
def fuse_lora_weights(
    ...
    for key in _affected_weight_keys(lora_sd_and_strengths):
        ...
        weight = _to_device_safe(original_weight, fusion_device, fuse_rule.aggregation_dtype)
...
def _products_for_sd_key(
    ...
    for lsd, coef in lora_sd_and_strengths:
        if key_a not in lsd.sd or key_b not in lsd.sd:
            continue
        a = _to_device_safe(lsd.sd[key_a], device, dtype)
        b = _to_device_safe(lsd.sd[key_b], device, dtype)
