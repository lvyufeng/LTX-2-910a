...
_configure_host_cxx_includes() {
  ...
  ordered=""
  for path in "$gcc_include" "/usr/include/$triple" "/usr/include/$triple/c++/$version" "/usr/include/c++/$version" "/usr/lib/gcc/aarch64-linux-gnu/11/include" "/usr/include/aarch64-linux-gnu" "/usr/include/aarch64-linux-gnu/c++/11" "/usr/include/c++/11" "/usr/include"; do
    [[ -n "$path" && -d "$path" ]] || continue
    case ":$ordered:" in *":$path:"*) ;; *) ordered="${ordered:+$ordered:}$path" ;; esac
  done
  ...
  for path in "${include_parts[@]}"; do
    [[ -n "$path" && -d "$path" ]] || continue
    case ":$ordered:$extra:" in *":$path:"*) ;; *) extra="${extra:+$extra:}$path" ;; esac
  done
  ...
  for path in "${include_parts[@]}"; do
    [[ -z "$path" ]] && continue
    flag="-I$path"
    case " $flags " in *" $flag "*) ;; *) flags="${flags:+$flags }$flag" ;; esac
  done
  export CXXFLAGS="$flags"
}
if [[ ! -f "$PROJECT_DIR/build.sh" ]]; then
  echo "Generated AscendC project not found at $PROJECT_DIR. Run generate.sh first." >&2
  exit 1
fi
