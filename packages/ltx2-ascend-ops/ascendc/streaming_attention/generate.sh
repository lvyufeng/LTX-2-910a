...
cat <<EOF
Generated StreamingAttention AscendC project at:
  $PROJECT_DIR

Next steps:
  1. Implement op_host/streaming_attention.cpp tiling and shape inference.
  2. Implement op_kernel/streaming_attention.cpp online-softmax attention.
  3. Build with:
       cd "$PROJECT_DIR" && bash build.sh
  4. Install/export the custom OPP paths printed by the generated install script.
EOF
