...
def assert_num_frames(num_frames: int) -> None:
    """Assert that the number of video frames matches the VAE temporal scale."""
    if num_frames < 1 or (num_frames - 1) % 8 != 0:
        raise ValueError(
            f"Number of frames ({num_frames}) must be 8*k + 1 for k >= 0. "
            "Valid examples include 9, 17, 97, and 121."
        )
