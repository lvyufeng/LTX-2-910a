...
def parse_torch_dtype(dtype: str | None) -> torch.dtype | None:
    if dtype is None:
        return None
...
def detect_checkpoint_path(distilled: bool = False) -> str:
    ...
    pre.add_argument(flag, type=resolve_existing_path)
    known, _ = pre.parse_known_args()
    checkpoint_path = known.distilled_checkpoint_path if distilled else known.checkpoint_path
...
def add_reference_runtime_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--device",
        default=None,
        help=(
            "Override inference device, for example cpu, cuda, cuda:0, npu, or npu:0. "
            "If omitted, the default accelerator selection is used."
        ),
    )
    parser.add_argument(
        "--dtype",
        ...
        default=None,
        help=(
            "Override inference dtype. Defaults to the pipeline's existing dtype policy; "
            "use float32 with --device cpu for reference runs."
        ),
    )
    parser.add_argument(
        "--text-encoder-dtype",
        ...
        default=None,
        help=(
            "Override Gemma text encoder dtype independently from --dtype. "
            "Useful for CPU references where Gemma can be numerically unstable in float32."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the prompt embeddings processor device independently from --device. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the prompt embeddings processor dtype independently from --dtype. "
...
def default_1_stage_arg_parser(
    params: PipelineParams = LTX_2_3_PARAMS,
    *,
    reference_runtime_args: bool = False,
) -> argparse.ArgumentParser:
    ...
    if reference_runtime_args:
        add_reference_runtime_args(parser)
    parser.add_argument(
        "--negative-prompt",
        type=str,
...
def default_2_stage_arg_parser(
    ...
    *,
    ...
    parser = default_1_stage_arg_parser(params=params, reference_runtime_args=reference_runtime_args)
