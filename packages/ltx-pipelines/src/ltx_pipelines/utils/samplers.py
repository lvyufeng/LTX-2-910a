...
def euler_denoising_loop(
    ...
    for step_idx, _ in enumerate(tqdm(sigmas[:-1])):
        device = video_state.latent.device if video_state is not None else audio_state.latent.device
        with profile_section(f"denoise.step.{step_idx}", device):
        ...
        dump_tensor(f"denoise.step.{step_idx}.video_denoised", denoised_video)
        dump_tensor(f"denoise.step.{step_idx}.audio_denoised", denoised_audio)
        ...
        dump_tensor(f"denoise.step.{step_idx}.video_latent_after_step", video_state.latent if video_state is not None else None)
        dump_tensor(f"denoise.step.{step_idx}.audio_latent_after_step", audio_state.latent if audio_state is not None else None)
