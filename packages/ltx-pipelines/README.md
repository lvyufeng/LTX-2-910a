...
### CPU Reference Workflow

For correctness checks, run the original/shared one-stage pipeline on CPU and compare it with a matching Ascend run. This is a mechanics reference, not a quality benchmark: the script uses the smallest valid shape (`32x32`, `9` frames, `1` denoising step) so the 22B checkpoint has some chance of completing on CPU.
...
The shared `ti2vid_one_stage` and `ti2vid_two_stages` CLIs accept `--device` and `--dtype` for explicit reference runs. Use `--device cpu --dtype float32` for CPU reference. Do not use `--offload cpu`, `--quantization`, or `--compile` for this baseline; those are accelerator/memory optimization paths, not a clean CPU reference. Compare decoded frames rather than MP4 file hashes because container metadata and encoding can differ.
