...
- `TI2VidOneStagePipeline` and `TI2VidTwoStagesPipeline` accept optional constructor `dtype`. When omitted, both keep the historical `torch.float16` default. Their module CLIs expose `--device` and `--dtype` for CPU/Ascend reference runs; other pipeline CLIs keep their previous runtime selection surface.
