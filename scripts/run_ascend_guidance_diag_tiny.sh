#!/bin/bash
# Tiny NPU full-guidance diagnostic with the CPU/fp32 embeddings fix in place.
# Optional first arg: fp32 guidance flag (0/1). Dumps guidance pass outputs
# (cond/uncond/ptb/mod/pred) at step 0.

set -euo pipefail

GUIDANCE_FP32="${1:-0}"

eval "$(conda shell.bash hook)"
conda activate ltx2-npu

REPO_ROOT="/mnt/data/lvyufeng/LTX-2-910a"
OUTPUT_DIR="${REPO_ROOT}/outputs/reference"
SUFFIX="fp16guidance"
if [ "${GUIDANCE_FP32}" = "1" ]; then
    SUFFIX="fp32guidance"
fi
OUTPUT="${OUTPUT_DIR}/guidance_diag_ascend_tiny_s1_${SUFFIX}.mp4"
mkdir -p "${OUTPUT_DIR}"

if [ -f /usr/local/Ascend/cann-9.0.0/set_env.sh ]; then
    # shellcheck disable=SC1091
    source /usr/local/Ascend/cann-9.0.0/set_env.sh
fi

export PYTHONPATH="${REPO_ROOT}/packages/ltx-core/src:${REPO_ROOT}/packages/ltx-pipelines/src:${REPO_ROOT}/packages/ltx2-ascend/src:${PYTHONPATH:-}"
export LTX2_ASCEND_ATTENTION=eager
export PYTHONHASHSEED=0
export TOKENIZERS_PARALLELISM=false
export LTX2_GUIDANCE_FP32="${GUIDANCE_FP32}"
export LTX2_DEBUG_DUMP_DIR="${OUTPUT_DIR}/guidance_diag_ascend_tiny_${SUFFIX}"
export LTX2_DEBUG_DUMP_PREFIX="ascend_guided_tiny_${SUFFIX}"
export LTX2_DEBUG_DUMP_FILTER="prompt,diffusion_stage,guidance.step.0"

python -m ltx2_ascend.cli \
    --quality-preset standard \
    --pipeline one-stage \
    --device 0 \
    --devices 0,1,2,3 \
    --layerwise \
    --checkpoint /mnt/data/LTX-2.3/ltx-2.3-22b-dev.safetensors \
    --gemma-root /mnt/data/gemma-3-12b-it-qat-q4_0-unquantized \
    --prompt "A cinematic shot of a calm mountain lake at sunrise." \
    --height 32 \
    --width 32 \
    --frames 9 \
    --steps 1 \
    --seed 10 \
    --text-encoder-device cpu \
    --text-encoder-dtype bfloat16 \
    --embeddings-processor-device cpu \
    --embeddings-processor-dtype float32 \
    --random-draw-device cpu \
    --random-draw-dtype float16 \
    --video-cfg 3.0 \
    --video-stg 1.0 \
    --video-rescale 0.7 \
    --a2v 3.0 \
    --audio-cfg 7.0 \
    --audio-stg 1.0 \
    --audio-rescale 0.7 \
    --v2a 3.0 \
    --no-tiling \
    --output "${OUTPUT}"

echo "Ascend guidance diagnostic (${SUFFIX}) written to ${OUTPUT}"
