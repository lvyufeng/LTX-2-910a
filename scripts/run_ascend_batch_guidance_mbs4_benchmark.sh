#!/bin/bash
# Benchmark batched guidance after padding/mask support.
# Uses max-batch-size 4 to batch all guidance passes in one transformer call if memory allows.
...
OUTPUT="${OUTPUT_DIR}/batch_guidance_opt_mbs4_512x768_f73_s5_seed10.mp4"
mkdir -p "${OUTPUT_DIR}"
...
echo "Batch guidance mbs4 benchmark complete: ${OUTPUT}"
