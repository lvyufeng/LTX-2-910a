#!/bin/bash
# Profiling benchmark: same precision config as the good run, reduced to 5 steps
# with LTX2_ASCEND_PROFILE=1 to measure per-guidance-pass transformer cost.
# Per-pass time is constant across steps, so 5 steps is enough to characterize it.
...
OUTPUT="${OUTPUT_DIR}/perf_profile_512x768_f73_s5_seed10.mp4"
mkdir -p "${OUTPUT_DIR}"
...
echo "Profile run complete: ${OUTPUT}"
