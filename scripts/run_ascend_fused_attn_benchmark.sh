#!/bin/bash
# Benchmark with npu_fusion_attention (fused FlashAttention kernel).
# Same precision config as the good run, 5 steps for quick comparison.
# Only change: LTX2_ASCEND_ATTENTION=fused instead of eager.
...
OUTPUT="${OUTPUT_DIR}/fused_attn_512x768_f73_s5_seed10.mp4"
mkdir -p "${OUTPUT_DIR}"
...
export LTX2_ASCEND_ATTENTION=fused
...
echo "Fused attention benchmark complete: ${OUTPUT}"
